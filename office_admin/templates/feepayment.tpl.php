	...
	<tr>
		...
		<td>
			<table width="100%" border="0" cellspacing="2" cellpadding="0" align="center">
				...
				<td align="left" class="narmal" width="30%"><?php echo $studetails['pre_name']." ".$studetails['middle_name']." ".$studetails['pre_lastname']; ?></td>
...
<tr>
					...
					<td align="left" class="narmal">:</td>
					<td align="left" class="narmal"><?php if($newfepaid_det['paid_on']!="" && $newfepaid_det['paid_on']!="0000-00-00"){ echo date("d-M-Y", strtotime($newfepaid_det['paid_on']));/*func_date_conversion("Y-m-d","d/m/Y",$newfepaid_det['paid_on']);*/}else{echo "---";} ?></td>
			  </tr>
				<tr>
					<td align="left" class="narmal"><span id="internal-source-marker_0.08468187621509426">Status </span></td>
				  <td align="left" class="narmal">:</td>
					<td align="left" class="narmal"><?php if($studetails['status']=='inactive'){echo Transferred;}else{echo 'Active';} ?></td>
				  <td align="left" class="narmal">&nbsp;</td>
				</tr>
			...
			?>
							<tr>
								...
								<td align="left" class="narmal"><?php echo $eachfeedet['fee_particular'];?></td>
									...
									echo $feeamt;
                                                                        $total_fee += $eachfeedet['fee_amount'];
									?></td>
								<!--<td align="center" class="narmal"><?php //if($eachfeedet['fee_instalments']==10){echo "Monthly";}elseif($eachfeedet['fee_instalments']==1){echo "Yearly";}?></td>-->
							</tr>
		  <?php } } else { ?>
				<tr>
				  <td colspan="4" align="center" class="narmal">No Fees for this Class</td>
						  ...
						  <?php }  ?>	
                                                        <tr>
                                                            ...
                                                            <td align="right"><strong>Rs. <?php echo number_format($total_fee, 2, '.', ''); ?></strong></td>
                                                            <input type="hidden" id="total" value="<?php echo $total_fee; ?>" />
                                                            <!--<td></td>-->
                                                        </tr>
		  ...
		  <div id="paybutton" style="display:block">
                  ...
                  <table width="98%" align="center" style="border: solid thin;">
                        ...
                        <tr>
                           <td>
                               <table width="90%" align="center">
                                   <tr>
                                       ...
                                       <td>Amount</td>
                                       <td>:</td>
                                       <td>Rs. <input type="text" name="amount_payable" id="amount_payable" onkeyup="calcRemaining(); distributeFees();" onblur="calcRemaining(); distributeFees();" /></td>
                                   </tr>
                                   <tr>
                                       <td>2)</td>
                                       <td>Fine</td>
                                       <td>:</td>
                                       <td>Rs. <input type="text" name="fine_payable" id="fine_payable" onkeyup="calcRemaining(); distributeFees();" onblur="calcRemaining(); distributeFees();" /></td>
                                   </tr>
                                   <tr>
                                       <td></td>
                                       <td></td>
                           ...
                           <td>
                               <?php $remaining = $total_fee - ($db->getOne("SELECT SUM(feeamount) FROM es_feepaid WHERE studentid=".$studetails['es_preadmissionid']." AND fi_fromdate='".$from_finance."' AND fi_todate='".$to_finance."'")); ?>
                               <table width="90%" align="center">
                                   <tr>
                                       <td>
                                       ...
                                       <td>
                                            Rs. <input type="text" name="remaining" id="remaining" readonly value="<?php echo $remaining; ?>" />
                                       </td>
                                   </tr>
                                   <tr>
                                       <td>
                                           <strong>Fee Remaining after this payment:</strong>
                                       </td>
                                       <td>
                                            Rs. <input type="text" name="remaining_after_current" id="remaining_after_current" readonly value="" />
                                       </td>
                                   </tr>
                                   <tr>
                                       <td></td>
                  ...
                  <script>
                      ...
                      {
                            var current_payment = document.getElementById("amount_payable").value;
                            var remaining = document.getElementById("remaining").value;
                            var balance;
                            if(current_payment == "" || current_payment == null || isNaN(current_payment))
                                balance = remaining;
                            else
                  ...
                  <table width="98%" border="1" align="center" cellpadding="0" cellspacing="0" class="tbl_grid" >
                      <tr class="bgcolor_02" height="25">
                        <td align="left" class="admin" width="6%" style="text-align: center;">S.No</td>
                        <td align="left" class="admin" width="48%" style="padding-left: 20px;">Particulars</td>
                        <td align="left" class="admin" width="18%" style="padding-left: 20px;">Amount (Rs)</td>
                      </tr>
                      <?php $particular = $db->getRows("SELECT * FROM es_feemaster WHERE fee_class=".$prev_class." AND fee_fromdate='".$from_finance."' AND fee_todate='".$to_finance."'");
							foreach($particular as $p)
							{
								$total = $p['fee_amount'];
								$paid = $db->getOne("SELECT SUM(feeamount) FROM es_feepaid WHERE studentid=".$studetails['es_preadmissionid']." AND particularid=".$p['es_feemasterid']);
								$balance = $total - $paid;
								
								echo '<input type="hidden" class="balance" id="bal_'.$p['fee_particular'].'" value="'.$balance.'" />';
							}
                            
                            $count= 0;
							$fee_details = $db->getRows("SELECT * FROM es_feemaster WHERE fee_class='".$prev_class."' AND fee_fromdate='".$from_finance."' AND fee_todate='".$to_finance."'");
                            foreach($fee_details as $fee)
                            {
                                $count++;?>
                                <tr>
                                    <td align='center'><?php echo $count; ?></td>
                                    <td style="padding-left: 20px;"><?php echo $fee['fee_particular']; ?></td>
                                    <td class="distr" id="distr_<?php echo $fee['fee_particular']; ?>" style="padding-left: 20px;"></td>
                                    <input type="hidden" name="fp_distr[<?php echo $fee['es_feemasterid']; ?>]" class="fp_distr" id="fp_distr_<?php echo $fee['fee_particular']; ?>" value="" />
                                    <input type="hidden" class="feetypes" value="<?php echo $fee['fee_particular']; ?>" />
                                </tr>
                      <?php }
                      ?>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td style="padding-left: 20px; font-weight:bold;" id="total_distribution"></td>
                  ...
                  <script>
						function distributeFees()
						{
								...
								for(var i = 0; i < n; i++)
									feetype[i] = document.getElementsByClassName("feetypes")[i].value;
								...
								for(var i=0; i<n; i++)
								{
									if(entered_val > document.getElementById("bal_"+feetype[i]).value)
									{
										document.getElementById("distr_"+feetype[i]).innerHTML = document.getElementById("bal_"+feetype[i]).value;
										document.getElementById("fp_distr_"+feetype[i]).value = document.getElementById("bal_"+feetype[i]).value;
										entered_val = entered_val - document.getElementById("distr_"+feetype[i]).innerHTML;
									}
									else
									{
										document.getElementById("distr_"+feetype[i]).innerHTML = entered_val;
										document.getElementById("fp_distr_"+feetype[i]).value = entered_val;
			  ...
			  $allfees_det = $db->getrows($sql_fees_assigned);
				...
				if (count($allfees_det)>0){
					...
					foreach($allfees_det as $eachfeedet){//get student paid list
							 ...
							 $feepaid_records = $db->getone("SELECT COUNT(*) FROM es_feepaid_new WHERE es_preadmissionid='".$studetails['es_preadmissionid']."' AND financemaster_id='".$_SESSION['eschools']['es_finance_masterid']."' ");
							 
							 
				
				        if($feepaid_records ==0){
								
								if($eachfeedet['fee_instalments']==10){
					    ...
					    if($eachfeedet['fee_instalments']==10 && $noofinstpaid<10 ){
									if($eachfeedet['fee_particular']=='TUITION'){
              ...
              <?php 
                      if($feepaid_records ==0){
								...
								if($eachfeedet['fee_instalments']==10){
									...
									// nothing echo its a wrong code
									echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$inast_amt.''.$fip_month_arr[date('F')].'';
					    ...
					    if($eachfeedet['fee_instalments']==10&& $noofinstpaid<10 ){
								if($eachfeedet['fee_particular']=='TUITION'){
									...
									echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$amttopayfee_inst1.' '.$inst_to_pay1.'';
			...
			?>
              ...
              <script type="text/javascript">
					...
					function showTotalFineAmount()
					{
						...
						var fineAmount=parseFloat(document.getElementById('fine_amount').value);
						var totalFineAmount=totalAmount+fineAmount;
						document.getElementById('total_fine_amount').value=totalFineAmount;
					}
...
<table width="60%" border="0" cellspacing="0" cellpadding="0">	 
				...
				<tr>
				  ...
				  $due_amt_r = (int)$feetotamt-(int)$feeamtleft;
				   ...
				   if($total_amount!=0){ ?>
				  <input type="button" value="Pay balance Fee"  name="balance" id="balance" style="cursor:pointer; display:none;" class="bgcolor_02" />
				  <?php  } ?></td>
								...
								<td class="narmal"><input class="plain" name="dc1"  value="<?php
													  if (isset($dc1)){ 
															 ...
															 ?>"
															  ...
															  size="12" onfocus="this.blur()" readonly /><a href="javascript:void(0)" onclick="if(self.gfPop)gfPop.fStartPop(document.fee_search.dc1,document.fee_search.dc2);return false;" ><img class="PopcalTrigger" align="absmiddle" src="<?php echo JS_PATH ?>/DateRange/calbtn.gif" width="34" height="22" border="0" alt="" />
								</td>
								<td class="narmal">To</td>
							 	<td class="narmal"><input class="plain" name="dc2" value="<?php
													  if (isset($dc2)){ 
															 ...
															 ?>" size="12" onfocus="this.blur()" readonly /><a href="javascript:void(0)" onclick="if(self.gfPop)gfPop.fEndPop(document.fee_search.dc1,document.fee_search.dc2);return false;" ><img class="PopcalTrigger" align="absmiddle" src="<?php echo JS_PATH ?>/DateRange/calbtn.gif" width="34" height="22" border="0" alt="" /></a>
								</td>
								<td class="narmal">
...
?> 
				<tr>
					...
					<td class="narmal" align="left">&nbsp;<?php echo $eachrecord['pre_name']." ".$eachrecord['pre_lastname']; ?></td>
					<?php /*?><td class="narmal" align="right"><?php if($eachrecord['waidamt']>0){echo $_SESSION['eschools']['currency'].'&nbsp;'.number_format($eachrecord['waidamt'], 2, '.', '');}else{echo "---";} ?></td><?php */?>
...
?>		  
				...
				<tr><td colspan="1">&nbsp;</td><td  align="right" class="bgcolor_02">Sub Total </td><td  class="bgcolor_02" align="right" height="25"><?php if($waivedtotal>0){echo $_SESSION['eschools']['currency'].'&nbsp;'.number_format($waivedtotal, 2, '.', '');}else{echo "---";}?></td><td  class="bgcolor_02" align="right" height="25"><?php echo $_SESSION['eschools']['currency'].'&nbsp;'.number_format($subtotal, 2, '.', '');?></td></tr>
				<tr>
					<td colspan="4" class="narmal" align="center">
					<?php 
				...
				<tr>
				<td colspan="4" align="center" class="narmal"> No Records Found </td></tr>
		   <?php } ?>
                     ...
                     <td >
                     	<table width="100%" border="0" cellspacing="0" cellpadding="0">
                        	<tr>
                                ...
                                <td width="35%"><input class="plain" name="dc1" value="<?php if (isset($dc1)&&$dc1!=""){ echo $dc1;} ?>" size="12" onfocus="this.blur()" readonly /></td>
                    ...
                    <td >
                    	<table width="39%" border="0" cellspacing="0" cellpadding="0">
                        	<tr>
                                ...
                                <td width="10%"><input class="plain" name="dc2" value="<?php if (isset($dc2)&&$dc2!=""){ echo $dc2; }?>" size="12" onfocus="this.blur()" readonly />
                                </td>
    ...
    <td width="30%" >
                     	<table width="100%" border="0" cellspacing="0" cellpadding="0">
                        	<tr>
                                ...
                                <td width="12%"><input class="plain" name="dc1" value="<?php if (isset($dc1)&&$dc1!=""){ echo $dc1;} ?>" size="12" onfocus="this.blur()" readonly /></td>
	    ...
	    <td height="1" colspan="3" >
			...
			<?php if($action!="fee_paid_listprint"){?>
				<tr>  
                    ...
                    <td  height="35" colspan="7" align="left" valign="middle" class="narmal">
                    	<select name="feecategories"  style="width:150px;">
							 <option value="all">- - All- - </option>
                             <?php  foreach($fees_cat_det as $eachfee){ ?>
                             <option value="<?php echo $eachfee['fee_particular']; ?>" <?php if($feecategories==$eachfee['fee_particular']){ ?> selected                           ="selected"                        <?php } ?> >
                             <?php    echo $eachfee['fee_particular']; ?>
...
<tr><td colspan="10"> <table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<?php }?> 
					...
					?>
		  <tr>
							<td class="narmal" colspan="9"><b><?php echo $eachfee['particulartname']; ?></b></td>
			  </tr>
				<?php }?>
		  ...
		  <tr class="<?php echo $zibracolor;?>">
							...
							<td class="narmal" align="center"><?php echo $i;?></td>
							<td class="narmal" align="center"><?php echo date("d/m/Y", strtotime($eachfee['date']));//func_date_conversion("Y-m-d","d/m/Y",$eachfee['date']);?></td>
							<td class="narmal" align="center"><?php echo $eachfee['es_preadmissionid'];?></td>
							<td class="narmal"><?php echo $eachfee['pre_name']." ".$eachfee['pre_lastname'];?></td>
							<td align="center" class="narmal"><?php echo $eachfee['pre_fathername'];?></td>
							<td class="narmal"><?php echo $eachfee['es_classname'];?></td>
							<td><?php echo ucfirst($eachfee['es_paymentmode']);?></td>
						   <td class="narmal"><?php echo $rest = substr($eachfee['es_receiptno'], 8); ?></td>
				 ...
				 $chequetotal+=$eachfee['feeamount'];
						   ...
						   ?> </td>
						   <?php /*?><td class="narmal"><?php  echo $_SESSION['eschools']['currency']."&nbsp;".number_format($eachfee['fee_waived'], 2, '.', '');
				 ...
				 <tr><td colspan="8" class="narmal" align="right"><b>SUB-TOTAL</b></td>
							...
							<?php /*?><td   class="narmal" align="left"><?php echo $_SESSION['eschools']['currency']."&nbsp;".number_format($total_waived, 2, '.', '');?></td><?php */?>
				</tr>
					...
					<?php }
							...
							<?php /*?><td   class="narmal" align="left" ><?php echo $_SESSION['eschools']['currency']."&nbsp;".number_format($grandtotal_waived, 2, '.', '');?></td><?php */?>
				...
				<?php /*?><tr><td class="narmal" align="left" colspan="3"><b>GRAND&nbsp;TOTAL&nbsp;WAIVED&nbsp;AMOUNT</b></td>
...
&nbsp;&nbsp;<a href="javascript: void(0)" onclick="popup_letter('?pid=40&action=receiptpayment&ofid=<?php echo $each['fid']; ?>')" ><?php /*?><img src="images/b_browse.png" border="0" title="Print Payment Recipt" alt="Print Payment Recipt" /><?php */?></a>
  ...
  <tr>
    <td><table width="95%" border="0" cellspacing="2" cellpadding="0" align="center">
				<tr>
					...
					<td align="left" class="narmal">:</td>
					<td align="left" class="narmal"><?php echo date("d/m/Y", strtotime($receipt_det['paid_on']));//func_date_conversion("Y-m-d","d/m/Y",$receipt_det['paid_on']);  ?></td>
					<td align="left" class="narmal">&nbsp;</td>
				</tr>	
                <tr>
					<td align="left" class="narmal">Receipt No</td>
               ...
               <tr>
                 ...
                 <!--<tr>
					<td align="left" class="narmal">Fees of the month</td>
					<td align="left" class="narmal">:</td>
					<td align="left" class="narmal"><?php //echo func_date_conversion("Y-m-d","m/Y",$receipt_det['paid_on']);  ?></td>
				</tr>-->
                <!--<tr>
					<td align="left" class="narmal">Section</td>
					<td align="left" class="narmal">:</td>
...
?>
  ...
  <tr>
    <td><table width="100%" border="1" cellspacing="0" cellpadding="0" class="tbl_grid">
		    ...
		    <tr class="bgcolor_02" height="25">
					<td width="33%" align="center" valign="middle" class="admin">&nbsp;Total Amount(Amount+fine)</td>                            
...
<?php /*?>                <tr height="25">
				...
				<tr height="25">
					<td align="left"  colspan="3" style="padding-right:25px;" class="admin">Note:- Please keep this slip for your reference</td>
				</tr>
<?php } ?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td>
     <?php if($action=='print_fee_card'){
  ?>
    <table width="100%">
  
  <tr>
    <td>
    	<!-- This is my space now adwait -->
        <table width="100%">
    	<tr>
            <!------------------------------------------- Main script for card printing ------------------------------------------->
            <td>
            	
                
                <!----------------------------------------------- Student fee card display ------------------------------------------------>
                    	...
                    	<style>
							...
							#fcp_table_fee_card tr td
							{
								text-align: center;
								white-space: nowrap;
								padding: 5px 0px;
							}
							#fcp_table_fee_card #fcp_total td
							{
								font-weight:bold;
								background-color:rgba(204,204,204,1);
							}
                        </style>
						<table border="1" id="fcp_table_fee_card" style="width:100%; margin-top: 20px;">
                        	<tr class="bgcolor_02">
                            	<th>Receipt Date</th>
                                <?php	foreach($fcp_feemaster_particulars as $ffp)
										{
											echo "<th>".$ffp['fee_particular']."</th>";
										}?>
                                <th id='fcp_hover'>Fine</th>
                                <th>Fee Paid</th>
                                <th>Balance</th>
                                <th>Total Fee</th>
                            </tr>
					...
					<?php	for($i=1; $i<=$fcp_installments_paid; $i++)
							{
								...
								<tr>
									...
									<?php
										foreach($inst as $distr)
										{
											echo "<td>".$distr['feeamount']."</td>";
											$paid += $distr['feeamount'];
											$fcp_total += $distr['feeamount'];
										}
            ...
            <!------------------------------------------- End of Main script for card printing ------------------------------------------->
		</tr>
    </table>
    
    <!-- End of this is my space adwait -->
 ...
 <?php 
  ...
  <?php
 if(isset($get_fee_card) && count($errormessage)==0){
  ?>
  <tr>
    <td>
    <table width="100%">
								  ...
								  <tr>
									<td width="50%"><table width="100%" border="0">
										...
										<tr>
										  ...
										  <td align="left" valign="middle" class="admin">:</td>
										  <td align="left" valign="middle"><?php  if($stud_details['status']=='inactive'){ echo 'Transferred';}else{echo 'Active';} ?></td>
									  </tr>
								    </table></td>
									<td><table width="100%" border="0">
										  <tr>
  ...
  <tr class="fee_card" height="25">
	...
	;
	echo number_format($balance_amt,2,'.','');?></td>
...
<?php  if($stud_details['status']=='inactive'){ echo 'Transferred';}else{echo '---';} ?></td>
 ...
 <?php 	
    ...
    <?php 
	$balance_amt = $this_month_total - $newfee_arr_paid[$year_card][$month_arr[$sr]]
	- $paid_arr[$year_card][$month_arr[$sr]]
	 - $ins_fine_paid_arr[$year_card][$month_arr[$sr]]
	-	$misc_paid_arr[$year_card][$month_arr[$sr]]
	-	$trans_paid_arr[$year_card][$month_arr[$sr]]
...
<?php if($action=='classwise_fee_card'){
	$finyear = $db->getRows("SELECT * FROM es_finance_master ORDER BY es_finance_masterid DESC");
	?>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
         <td height="3" colspan="3"></td>
  ...
  <tr>
    <td><form method="post" action="" name="fetchstudent">
			<div><div >&nbsp;</div>
				...
				<span class="narmal">&nbsp;&nbsp;&nbsp;
					<select name="cfc_fin_year">
                    	<?php	foreach($finyear as $fy)
								{ ?>
                    				<option value = "<?php echo $fy['es_finance_masterid']; ?>" <?php if($fy['es_finance_masterid'] == $cfc_fin_year){echo "selected";} ?>><?php echo $fy['fi_startdate']." to ".$fy['fi_enddate']; ?></option>
                        <?php	} ?>
                    </select>
                </span>&nbsp;&nbsp;&nbsp;&nbsp;
  ...
  <tr>
  <td style="padding: 10px 0px 10px 8px;">
        <b>NOTE:</b> Students who are displayed in Red are Transferred Students.
        <br><br>
  </td>
  </tr>
  ...
  ?>
  <tr class="fee_card" height="25">
    <td align="center"><?php echo $each_student['roll_no'];?></td>
  <?php  
	...
	if($trans==0){
?>
    <td align="center"><?php echo $each_student['es_preadmissionid'];?></td>
    <?php }else{?>
     <td align="center" bgcolor="#FF0000" >&nbsp;<?php echo $each_student['es_preadmissionid'];?></td>
    <?php }?>
    <td align="center"><?php echo $each_student['section_name'];?></td>
    <td align="center"><?php echo $each_student['pre_name'];?></td>
    <?php 
	$month_total = 0;
	...
	{?>
    	<!--<td align="center">-->
		<?php 
			if($eachfee['fee_particular']=='TUITION')
			{
				$amount_tt = $eachfee['fee_amount']-($each_student['fee_concession']);
				//echo $amount_tt ;
			}
			else
			{
				$amount_tt = $eachfee['fee_amount'];
				//echo $amount_tt;
			}?>
		<!--</td>-->
		<?php
	...
	<?php
        foreach($fee_det as $eachfee)
        {
            $feepaid_det = $db->getOne("SELECT SUM(feeamount) FROM es_feepaid WHERE studentid=".$each_student['es_preadmissionid']." AND particularid=".$eachfee['es_feemasterid']);
			?>
            <td align="center"><?php echo $feepaid_det; ?></td>
<?php	}
	?>
    
	<?php // Previous code deleted, can be found inside cp_11th_Nov ?>
    <td align="center"><?php echo $installmentfine_topay[$each_student['es_preadmissionid']];?></td>
 
    <td align="center">
	
    <?php
    	$feepaid = $db->getOne("SELECT SUM(paid) FROM es_feepaid_new WHERE es_preadmissionid=".$each_student['es_preadmissionid']." AND financemaster_id=".$cfc_fin_year);
		echo $feepaid;
		$grand_feepaid += $feepaid;
	?>
	...
	// Previous code deleted, can be found inside cp_11th_Nov
?></td>
    <td align="center">
	<?php
		$fee_for_class = $db->getOne("SELECT SUM(fee_amount) FROM es_feemaster WHERE fee_class=".$pre_class." AND fee_fromdate='".$_SESSION['fi_startdate']."'");
    	$balance = $db->getOne("SELECT balance FROM `es_feepaid_new` WHERE `es_preadmissionid`=".$each_student['es_preadmissionid']." AND financemaster_id=".$_SESSION['finance_masterid']." AND `fid`= (SELECT MAX(fid) FROM `es_feepaid_new` WHERE `es_preadmissionid`=".$each_student['es_preadmissionid'].")");
		if($balance == "")
			echo $fee_for_class;
		else
			echo $balance;
	?>
	<?php $balance_amt =
	// $all_fees_topay_amt - $student_paid[$each_student['es_preadmissionid']]
	$all_fees_topay_amt -
  ...
  <?php /*?><tr align="center" height="30px" class="fee_card">
  	<td colspan="4">Total</td>
  <?php	foreach($fee_det as $eachfee)
        {?>
			<td></td>
  <?php }?>
  	<td></td>
  	<td><?php echo $grand_feepaid; ?></td>
  	<td></td>
  	<td><?php echo $grand_fee_for_class; ?></td>
  </tr><?php */?>
...
<?php	}
	?>
    <?php /*?><td align="center">&nbsp;<?php echo $misc_topay[$each_student['es_preadmissionid']];?></td><?php */?>
    <?php /*?><td align="center">&nbsp;<?php echo $tr_topay[$each_student['es_preadmissionid']];?></td><?php */?>
    <?php /*?><td align="center">&nbsp;<?php echo $stationary_topay[$each_student['es_preadmissionid']];?></td><?php */?>
    <?php /*?><td align="center">&nbsp;<?php echo $lib_topay[$each_student['es_preadmissionid']];?></td><?php */?>
    <?php /*?><td align="center">&nbsp;<?php echo $hostel_topay[$each_student['es_preadmissionid']];?></td><?php */?>
    <?php /*?><td align="center">&nbsp;<?php echo $examfee_topay[$each_student['es_preadmissionid']];?></td><?php */?>
    <td align="center"><?php echo $installmentfine_topay[$each_student['es_preadmissionid']];?></td>
    <?php /*?><td align="center">&nbsp;<?php echo $balance_topay[$each_student['es_preadmissionid']];?></td><?php */?>
    
    <td align="center">
	<?php
    	$feepaid = $db->getOne("SELECT SUM(paid) FROM es_feepaid_new WHERE es_preadmissionid=".$each_student['es_preadmissionid']);
		echo $feepaid;
	?>
	<?php
...
if($installmentfine_paid[$each_student['es_preadmissionid']]>0 ||
	...
	/*$all_fees_waived_amt = $student_waived[$each_student['es_preadmissionid']]+$misc_waived[$each_student['es_preadmissionid']]+$tr_waived[$each_student['es_preadmissionid']]+$stationary_waived[$each_student['es_preadmissionid']]+$lib_waived[$each_student['es_preadmissionid']]+$hostel_waived[$each_student['es_preadmissionid']]+$installmentfine_waived[$each_student['es_preadmissionid']]+$balance_waived[$each_student['es_preadmissionid']];
	if($all_fees_waived_amt>0){ echo $all_fees_waived_amt;}*/?></td>
    <td align="center">
    <?php
    	$balance = $db->getOne("SELECT balance FROM `es_feepaid_new` WHERE `es_preadmissionid`=".$each_student['es_preadmissionid']." AND `fid`= (SELECT MAX(fid) FROM `es_feepaid_new` WHERE `es_preadmissionid`=".$each_student['es_preadmissionid'].")");
		echo $balance;
	?>
	...
	if($all_fees_paid_amt>0){ echo $all_fees_paid_amt;}*/?></td>
    <td align="center">
	<?php $fee_for_class = $db->getOne("SELECT SUM(fee_amount) FROM es_feemaster WHERE fee_class=".$pre_class); echo $fee_for_class; ?>
	<?php /*$all_fees_topay_amt =  $month_total+$misc_topay[$each_student['es_preadmissionid']]+$tr_topay[$each_student['es_preadmissionid']]+$stationary_topay[$each_student['es_preadmissionid']]+$lib_topay[$each_student['es_preadmissionid']]+$hostel_topay[$each_student['es_preadmissionid']]+$examfee_topay[$each_student['es_preadmissionid']]+$installmentfine_topay[$each_student['es_preadmissionid']]+$balance_topay[$each_student['es_preadmissionid']];
	if($all_fees_topay_amt>0){ echo $all_fees_topay_amt;}*/?></td>
...
?>
	<table width="800px">
    	<thead>
        	<th class="bgcolor_02" style="padding: 5px 5px 5px 5px; text-align:left">Student Fee Card</th>
        </thead>
    </table>
	...
	<table width="100%">
    	<tr>
            	...
            	<table align="center">
                	<tr>
                        ...
                        <td><select name="fcp_fin_year">	<!--Financial year select box-->
                        		<?php	foreach($finyear as $f)
										{?>
                        					<option value="<?php echo $f['es_finance_masterid']; ?>"><?php echo $f['fi_startdate']." to ".$f['fi_enddate']; ?></option>
                                <?php	}?>
                        	</select></td>
                        <td><input type="submit" name="fcp_search_student" value="GO" class="bgcolor_02" /></td>
                    </tr>
                </table>
                </form>
                <!----------------------------------------------- End of Searching student ------------------------------------------------>
			...
			<?php	}	// End of Student has fee paid record
				}	// End of isset($fcp_search_student)
				?>
                <!--------------------------------------------- End of student fee card display ------------------------------------------>
            </td>
            <!------------------------------------------- End of Main script for card printing ------------------------------------------->
        ...
        <tr height="1px" class="bgcolor_02">
        	<td></td>
            <td></td>
            <td></td>
		</tr>
...
<?php
}	// End of $action == "ad_fee_card"
//--------------------------------------------------------- End of Fee Card Printing ----------------------------------------------------------//
?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
    	<td height="25" colspan="3" class="bgcolor_02">&nbsp;&nbsp;<span class="admin">Class wise Students Fee status</span></td>
    </tr>
	<tr>
		<td width="1" class="bgcolor_02"></td>
		<td align="left" valign="top">
        ...
        <table width="100%">
  			<tr>
<?php
			if($action != "ad_classwise_fee_card_print")
			{
?>
    			<td>
...
<?php 
                        ...
                        {
?>
                            <option <?php if($indclass['es_classesid']==$cfc_pre_class) { echo "selected='selected'"; } ?> value="<?php echo $indclass['es_classesid']; ?>"><?php echo $indclass['es_classname']; ?></option>
<?php
                        }
?>
				</select>&nbsp;
                <!------------------------------ End: 'class' select tag ------------------------------>
                
                <!------------------------------ Start: 'Financial Year' select tag ------------------------------>
				<span class="narmal">&nbsp;&nbsp;&nbsp;
				<select name="cfc_fin_year">
<?php
...
?>
                ...
                <!------------------------------ End: 'Financial Year' select tag ------------------------------>
                
				<input type="submit" name="submit_fee_status" value="submit" class="bgcolor_02" style="padding-left:10px;padding-right:10px;cursor:pointer;"/></div>
			</form><br>
            </td>
<?php
		} // End of if($action != "ad_classwise_fee_card_print")
?>
		</tr>
        
<?php
		if(isset($submit_fee_status))	// Submit button clicked; code to display details
		{
?>
		<tr>
        	<td>
...
?>
				<table width="100%">
                	<tr align="center">
                    	<td width="50%">Class: <strong><?php echo $db->getOne("SELECT es_classname FROM es_classes WHERE es_classesid=".$cfc_pre_class); ?></strong></td>
                        <td width="50%">Year: <strong><?php echo date("Y", strtotime($fi_startdate))."</strong> - <strong>".date("Y", strtotime($fi_enddate)); ?></strong></td>
                    </tr>
                </table>
            	<!---------------------------- Table of details ---------------------------->
                <table width="100%" border="1" style="border-collapse:collapse;">
                	<tr class="bgcolor_02" align="center">	<!-- Table headers -->
                    	<th>Reg. No.</th>
                        <th>Name</th>
<?php
						foreach($fee_particulars as $fp)
						{
?>
						<th><?php echo $fp['fee_particular']; ?></th>
<?php							
						}
?>
                        <th>Fine</th>
...
<?php                    
     				foreach($preadmission_details as $pre_det)	// Prints data for every student
					{
						// Gets the admission details from 'es_preadmission' associated with the current student
						$preadmission = $db->getRow("SELECT * FROM es_preadmission WHERE es_preadmissionid=".$pre_det['es_preadmissionid']);
?>               
                    <tr>	<!-- Table Data -->
                    	<td align="center"><?php echo $pre_det['es_preadmissionid']; ?></td>
                        <td align="center"><?php echo ucwords($preadmission['pre_name']." ".$preadmission['pre_lastname']);?></td>
<?php
						foreach($fee_particulars as $fp)
						{
							// Gets the amount paid for each fee particular
							$feepaid = $db->getOne("SELECT SUM(feeamount) FROM es_feepaid WHERE studentid=".$pre_det['es_preadmissionid']." AND particularid=".$fp['es_feemasterid']." AND fi_fromdate='".$fi_startdate."'");
?>
						<td align="center"><?php echo $feepaid; ?></td>
<?php
						}
						
						// Gets the total fine paid by current student in selected class and financial year
						$fine = $db->getOne("SELECT SUM(fine_amount) FROM es_fine_charged_collected WHERE studentid=".$pre_det['es_preadmissionid']." AND class=".$cfc_pre_class." AND fi_fromdate='".$fi_startdate."'");
						
						// Gets the total fee paid by current student (including fine) in selected class and financial year
						$total_fee_paid = $db->getOne("SELECT SUM(paid) FROM es_feepaid_new WHERE es_preadmissionid=".$pre_det['es_preadmissionid']." AND financemaster_id=".$cfc_fin_year." AND class_id=".$cfc_pre_class);
?>
						<td align="center"><?php echo $fine; ?></td>
                        <td align="center"><?php echo $total_fee_paid; ?></td>
                        <td align="center"><?php echo ($total_fee_for_class - ($total_fee_paid - $fine)); ?></td>
                        <td align="center"><?php echo $total_fee_for_class; ?></td>
                    </tr>	<!-- End of table Data -->
<?php
					} // End of foreach($preadmission_details as $pre_det)
?>
                </table>
                <!------------------------- End of table of details ------------------------>
                
<?php
			if($action != "ad_classwise_fee_card_print")
			{
?>
                <div width="100%" align="right"><br>
                	<input type="button" style="cursor:pointer;" value="Print" onclick="window.open('?pid=40&action=ad_classwise_fee_card_print&cfc_fin_year=<?php echo $cfc_fin_year; ?>&cfc_pre_class=<?php echo $cfc_pre_class; ?>&submit_fee_status=submit',null,'width=700,height=600,scrollbars=yes,toolbar=no,directories=no,status=no,menubar=yes,left=140,top=30');" class="bgcolor_02"  />
                </div>
<?php
			}
?>
<?php
			} // End of if($is_paid_fee > 0)
			else
			{
				echo "<div width='100%' align='center'>No records found</div>";
			}
?>
            </td>
        </tr>
<?php
		} // End of if(isset($submit_fee_status))
?>        
