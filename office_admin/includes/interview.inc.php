...
if($action == 'interview_registration')
{
   ...
   if(count($errormessage) == 0)
	...
	if (isset($eachrecord1->st_marital)){
		$es_staff ->st_marital = $eachrecord1->st_marital;
		
	}
	if (isset($eachrecord1->st_experience)){
		$es_staff ->st_experience = $eachrecord1->st_experience;
		
	}
	if (isset($eachrecord1->st_category)){
		$es_staff ->st_category = $eachrecord1->st_category;
	...
	if (isset($eachrecord1->st_faminfo)){
		$es_staff ->st_faminfo = $eachrecord1->st_faminfo;
		
	}
	if (isset($eachrecord1->st_hobbies)){
		$es_staff ->st_hobbies = $eachrecord1->st_hobbies;
		
	}
	if (isset($eachrecord1->st_attach1)){
		$es_staff ->st_attach1 = $eachrecord1->st_attach1;
		
	}
	if (isset($eachrecord1->st_attach2)){
		$es_staff ->st_attach2 = $eachrecord1->st_attach2;
		
	}
	if (isset($eachrecord1->st_attach3)){
		$es_staff ->st_attach3 = $eachrecord1->st_attach3;
		
	}
	if (isset($eachrecord1->st_attach4)){
		$es_staff ->st_attach4 = $eachrecord1->st_attach4;
   ...
   if ($insertedid = $es_staff ->Save())
	...
	$log_insert_exe=mysql_query($log_insert_sql);
		...
		header('location:?pid=15&action=applicants_list&emsg=42');
		  }
		}
	   }
	}
...
/********************** Back ***************************/	
	...
	if (isset($back)){
		header('location:?pid=15&action=applicants_list');
	}
/*************** End of Back Button -*******************/
...
/***************** View Listing ************************/
	...
	if (isset($takainterview)){
		header('location: ?pid=15&action=interview_registration');
	}
...
// The "i" after the pattern delimiter indicates a case-insensitive search
	...
	if ($error==""){
		...
		$db->update('es_staff',
			...
			st_noofsons            ='" . $st_son . "',
			st_marital             ='" . $st_maritalstatus . "',
			st_experience          ='" . $st_exp . "',
			st_category            ='" . $st_categ . "',
			st_email               ='" . $st_email . "',
			st_examp1              ='" . $st_pass1 . "',
			st_examp2              ='" . $st_pass2 . "',
			st_examp3              ='" . $st_pass3 . "',
			...
			st_faminfo             ='" . $st_faminfo . "',
			st_hobbies             ='" . $st_hobbies . "',
			st_attach1             ='" . $st_attach1 . "',
			st_attach2             ='" . $st_attach2 . "',
			st_attach3             ='" . $st_attach3 . "',
			st_attach4             ='" . $st_attach4 . "',
			st_prcity              ='" . $st_city . "',
			st_prpincode           ='" . $st_pin . "',
		...
		header('location: ?pid=15&action=applicants_list&emsg=36');
...
if($_FILES["photo_upload"]["name"] == ''){
	...
	$st_dob = formatDateCalender($st_dob,"Y-m-d");
    	$st_doj2 = formatDateCalender($st_doj2,"Y-m-d");
...
header("location: ?pid=15&action=staffviewing&emsg=2&start=$start&action1=selsearch&column_name=es_staffid");
...
if(isset($updateinterview))
{
	...
	$st_doj1 = func_date_conversion("d/m/Y","Y-m-d",$st_doj);
		...
		header('location: ?pid=15&action=applicants_list&emsg=2');
		}
...
/********************** Delete Staff *****************************/
	...
	$es_staff->Delete();
	$emsg = 3;
	header('location:?pid=15&action=staffviewing&emsg'.$emsg);
	exit;
