...
$address=$student_det['pre_address1'];
...
if($action=='list'){
		...
		$sql_qr = "SELECT * FROM es_bonafiedbank where status !='Deleted' ";
...
$db->update("es_bonafiedbank","status='Deleted'",'id='.$id );
	header("location: ?pid=120&action=list&emsg=3");
	}
  ...
  //array_print($_POST);
   ...
   if($hobbies==''){ $errormessage['hobbies'] = "Select  Class";}
    if($es_preadmissionid==''){ $errormessage['es_preadmissionid'] = "Select Student ID";}
	 ...
	 $count="select count(*) from  es_bonafiedbank where   sno='".$es_preadmissionid."' and status !='Deleted' ";
$c =$db->getOne($count);
...
//  if($c!=0){$errormessage['student_name']  = "Student Name Already Exist";}
	 ...
	 $count="select count(*) from  es_bonafiedbank where   student_name='".$es_preadmissionid."' and status !='Deleted' ";
$c =$db->getOne($count);
  ...
  if($gender==''){ $errormessage['gender'] = "Select Gender";}
	...
	*/		 
		...
		$dob=func_date_conversion("d/m/Y","Y-m-d",$dob);
					  ...
					  $db->insert("es_bonafiedbank",$message_arr);
					  header("location:?pid=120&action=list&emsg=1");	
       }
    }
}
...
if($action=='edit'){
	...
	$sem_det = $db->getrow("SELECT * FROM es_bonafiedbank WHERE id=".$id);
  ...
  // if($es_classesid==''){ $errormessage['es_classesid'] = "Select  Branch";}
   if($hobbies==''){ $errormessage['hobbies'] = "Select  Class";}
		...
		if($dob==''){ $errormessage['dob'] = "Enter Student Date Of Birth";}
			...
			$dob=func_date_conversion("d/m/Y","Y-m-d",$dob);
			if(count($errormessage)==0){
			//	$db->update("es_bonafiedbank","sno='".$sno1."',gender='".$gender."',father_name ='".$father_name ."',mother_name ='".$mother_name ."',section  ='".$section."',exam_date='".$exam_date."',roll_number='".$rollnumber."',marks_obtained='".$marks_obtained."',rank='".$rank."',dob='".$dob."',charector='".$charector."',conduct='".$conduct."',games='".$games."',hobbies='".$hobbies."'	,exam_name='".$exam_name."'","sno=".$id);
				
				
				 $query="update es_bonafiedbank set gender='".$gender."',father_name ='".$father_name ."',mother_name ='".$mother_name ."',section  ='".$section."',exam_date='".$exam_date."',roll_number='".$rollnumber."',marks_obtained='".$marks_obtained."',rank='".$rank."',dob='".$dob."',charector='".$charector."',conduct='".$conduct."',games='".$games."',hobbies='".$hobbies."'	,exam_name='".$exam_name."' where id=".$id;
				
				mysql_query($query);
				header("location:?pid=120&action=list&emsg=2");
				exit;
			}
...
if($action=='print_bonafied_bankcertificate'){
	$candidate_det_01 = $db->getrow("SELECT * FROM es_bonafiedbank WHERE id=".$id);
