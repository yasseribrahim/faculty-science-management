...
if($action=="searchschool")
{
	...
	{
		 header("Location:?pid=5&action=pre_admission");
		 exit;
	}
...
*/
	...
	$pre_image = $_FILES['pre_image']['name'];
		...
		$pre_dateofbirth = date("Y-m-d", strtotime(str_replace("/", "-", $pre_dateofbirth)));//func_date_conversion("d/m/Y","Y-m-d",$pre_dateofbirth);
